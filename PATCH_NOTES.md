...
## v0.1.9 - 2026-01-21 - 図形プレビューの改善とデザイン統一

**ステータス**: アルファ版 (Preview Update)

### 変更点

- **図形プレビューの修正**:
  - 四角形 (Rectangle) と円 (Circle) ツールで、作成中のプレビューが表示されない不具合を修正しました。
  - 作成途中の形状がリアルタイムで確認可能になりました。

- **デザインの統一**:
  - 多角形 (Poly)、四角形、円のすべてのツールでプレビューデザインを統一しました。
  - **視認性向上**: 建設予定地として「Teal (青緑)」色の半透明塗りつぶしと「Yellow (黄)」色の枠線を採用しました。
  - **情報表示**: 各辺の長さや半径、角度のラベル表示を見やすく改善しました。
...
- **プレビュー機能の強化**:
  - 作成中に図形の「長さ（m）」や「半径（R: m）」が数値で表示されるようになりました。
  - プレビューの見た目を改善（半透明の塗りつぶし＋明確な境界線）し、完成形がイメージしやすくなりました。
